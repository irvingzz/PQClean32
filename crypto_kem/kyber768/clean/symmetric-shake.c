...
*              - uint32_t outlen: number of requested output bytes
*              - const uint8_t *key: pointer to the key (of length KYBER_SYMBYTES)
*              - uint8_t nonce: single-byte nonce (public PRF input)
**************************************************/
void PQCLEAN_KYBER768_CLEAN_kyber_shake256_prf(uint8_t *out, uint32_t outlen, const uint8_t key[KYBER_SYMBYTES], uint8_t nonce) {
