...
int main(void) {
    ...
    uint8_t mi[MAXMLEN];
    uint8_t sig[PQCLEAN_DILITHIUM2_CLEAN_CRYPTO_BYTES];
    ...
    // printf("   - pk:\n");
    // print_hex(pk,PQCLEAN_DILITHIUM2_CLEAN_CRYPTO_PUBLICKEYBYTES);
    // printf("\n");
    // printf("   - sk:\n");
    // print_hex(sk,PQCLEAN_DILITHIUM2_CLEAN_CRYPTO_SECRETKEYBYTES);
    // printf("\n");

    cycles_signature = perf_rdcycle();
    instrs_signature = perf_rdinstret();
    PQCLEAN_DILITHIUM2_CLEAN_crypto_sign_signature(sig, &siglen, mi, i, sk);
    ...
    r = PQCLEAN_DILITHIUM2_CLEAN_crypto_sign_verify(sig, siglen, mi, i, pk);
    cycles_verify = perf_rdcycle() - cycles_verify;
    instrs_verify = perf_rdinstret() - instrs_verify;
    if (r) {
