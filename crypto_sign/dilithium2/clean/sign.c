...
void print_poly(poly* s, uint32_t len)
{
    for (uint32_t i=0; i<len; i++,s++)
    {
        for (uint32_t j=0; j<N; j++)
        {
           printf("%04x",s->coeffs[j]);
        }
        printf("\n");
    }
    printf("\n");
...
int PQCLEAN_DILITHIUM2_CLEAN_crypto_sign_keypair(uint8_t *pk, uint8_t *sk) {
    ...
    // printf("   - seedbuf:\n");
    // print_hex(seedbuf,SEEDBYTES);
    // printf("\n");
    shake256(seedbuf, 2 * SEEDBYTES + CRHBYTES, seedbuf, SEEDBYTES);
    // printf("   - seedbuf:\n");
    // print_hex(seedbuf,2 * SEEDBYTES + CRHBYTES);
    // printf("\n");

    // 2022.07.15, 20:06, I checked the code here and ensured the correctness of the code above.
    // From this piont, the following code is waitting for checking.
...
*              - uint32_t *siglen: pointer to output length of signature
...
*                             can be equal to m
*              - uint32_t *smlen: pointer to output length of signed
*                               message
...
*              - uint32_t siglen: length of signature
...
*              - uint32_t *mlen: pointer to output length of message
*              - const uint8_t *sm: pointer to signed message
*              - uint32_t smlen: length of signed message
*              - const uint8_t *pk: pointer to bit-packed public key
*
