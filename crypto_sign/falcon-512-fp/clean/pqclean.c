 ...
 */
static int
do_sign(uint8_t *nonce, uint8_t *sigbuf, uint32_t *sigbuflen,
    ...
    // printf("\t\tI am here at the beginning!\n");
    /*
...
static int
do_verify(
    const uint8_t *nonce, const uint8_t *sigbuf, uint32_t sigbuflen,
...
PQCLEAN_FALCON512_CLEAN_crypto_sign_open(
    ...
    uint32_t pmlen, sigbuflen;

    if (smlen < 3 + NONCELEN) {
        return -1;
    }
    sigbuflen = ((uint32_t)sm[0] << 8) | (uint32_t)sm[1];
